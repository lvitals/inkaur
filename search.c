...
static void print_info_field(const char *label, struct json *pkg, const char *key) {
    struct json *item = json_get_dict_item(pkg, (char *)key);
    if (!item) {
        printf("%-16s: None\n", label);
        return;
    }
    ...
    if (item->type == json_array) {
        printf("%-16s:", label);
        for (int i = 0; i < item->n_data_items; i++) {
            struct json *val = json_get_array_item(item, i);
        ...
        printf("%-16s: %s\n", label, item->data.string ? item->data.string : "None");
    } else if (item->type == json_number) {
        if (strstr(key, "Date") || strstr(key, "Submitted") || strstr(key, "Modified")) {
            time_t t = (time_t)item->data.number;
...
void show_package_info(const char *package_name) {
    char url[1024];
    snprintf(url, sizeof(url), "https://aur.archlinux.org/rpc/?v=5&type=info&arg=%s", package_name);

    struct rpc_data *data = make_rpc_request(url);
    if (!data || data->resultcount == 0) {
        fprintf(stderr, "error: package '%s' not found\n", package_name);
        if (data) free_rpc_data(data);
        return;
    }

    struct json *pkg = json_get_array_item(data->results, 0);
    if (!pkg) {
        free_rpc_data(data);
        return;
    }
    ...
    print_info_field("Name", pkg, "Name");
    print_info_field("Version", pkg, "Version");
    print_info_field("Description", pkg, "Description");
    print_info_field("URL", pkg, "URL");
    print_info_field("Licenses", pkg, "License");
    print_info_field("Votes", pkg, "NumVotes");
    print_info_field("Popularity", pkg, "Popularity");
    print_info_field("Maintainer", pkg, "Maintainer");
    print_info_field("First Submitted", pkg, "FirstSubmitted");
    print_info_field("Last Modified", pkg, "LastModified");
